    ...
    for item in itemlist:
        ...
        if attr_val == 900: # pomijamy kilka pierwszych wyników (rozgrzewka)
            ...
            if zeros != 0 and zeros != 2 and zeros != 3 and zeros != 6:
                file_res.append((mean_av/item_ctr, mean_arv/item_ctr))
        ...
        mean_av += float(item.attributes['avg_velocity'].value.replace(',', '.'))
        mean_arv += float(item.attributes['avg_riding_velocity'].value.replace(',', '.'))
